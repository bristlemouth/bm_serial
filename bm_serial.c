...
/*!
  ...
  \param[in] *callbacks pointer to callback structure. This file keeps it's own
  copy \return none
*/
...
*/
static bm_serial_error_e _bm_serial_validate_topic_and_cb(const char *topic,
                                                          uint16_t topic_len) {
...
static bm_serial_error_e _bm_serial_sub_unsub(const char *topic, uint16_t topic_len, bool sub) {
  ...
  do {
    ...
    uint16_t message_len =
        sizeof(bm_serial_packet_t) + sizeof(bm_serial_pub_header_t) + topic_len;
...
bm_serial_error_e bm_serial_cfg_status_response(uint64_t node_id, BmConfigPartition partition,
  ...
  do {
    ...
    bm_serial_packet_t *packet =
        _bm_serial_get_packet(BM_SERIAL_CFG_STATUS_RESP, 0, message_len);
...
bm_serial_error_e bm_serial_cfg_clear_request(uint64_t node_id, BmConfigPartition partition) {
  bm_serial_error_e rval = BM_SERIAL_OK;
  do {
    uint16_t message_len = sizeof(bm_serial_packet_t) + sizeof(BmConfigClearRequest);
    bm_serial_packet_t *packet = _bm_serial_get_packet(BM_SERIAL_CFG_CLEAR_REQ, 0, message_len);

    if (!packet) {
      rval = BM_SERIAL_OUT_OF_MEMORY;
      break;
    }
    BmConfigClearRequest *clear_req = (BmConfigClearRequest *)packet->payload;
    clear_req->header.target_node_id = node_id;
    clear_req->header.source_node_id = 0; // UNUSED.
    clear_req->partition = partition;
    packet->crc16 = bm_serial_crc16_ccitt(0, (uint8_t *)packet, message_len);
    if (!_callbacks.tx_fn((uint8_t *)packet, message_len, BM_SERIAL_CFG_CLEAR_REQ)) {
      rval = BM_SERIAL_TX_ERR;
      break;
    }
  } while (0);
...
bm_serial_error_e bm_serial_cfg_clear_response(uint64_t node_id, BmConfigPartition partition,
  ...
  bm_serial_error_e rval = BM_SERIAL_OK;
  do {
    uint16_t message_len = sizeof(bm_serial_packet_t) + sizeof(BmConfigClearResponse);
    bm_serial_packet_t *packet =
        _bm_serial_get_packet(BM_SERIAL_CFG_CLEAR_RESP, 0, message_len);

    if (!packet) {
      rval = BM_SERIAL_OUT_OF_MEMORY;
      break;
    }
    BmConfigClearResponse *clear_resp = (BmConfigClearResponse *)packet->payload;
    clear_resp->header.target_node_id = 0; // UNUSED
    clear_resp->header.source_node_id = node_id;
    clear_resp->partition = partition;
    clear_resp->success = success;
    packet->crc16 = bm_serial_crc16_ccitt(0, (uint8_t *)packet, message_len);
    if (!_callbacks.tx_fn((uint8_t *)packet, message_len, BM_SERIAL_CFG_CLEAR_RESP)) {
      rval = BM_SERIAL_TX_ERR;
      break;
    }
  } while (0);
...
bm_serial_error_e bm_serial_send_info_request(uint64_t node_id) {
  ...
  do {
    ...
    bm_serial_device_info_request_t *device_info_req_msg =
...
bm_serial_error_e bm_serial_send_info_reply(uint64_t node_id,
  ...
  do {
    ...
    bm_serial_device_info_reply_t *device_info_reply_msg =
        (bm_serial_device_info_reply_t *)packet->payload;
    memcpy(device_info_reply_msg, bcmp_info,
...
bm_serial_error_e bm_serial_send_resource_request(uint64_t node_id) {
  ...
  do {
    ...
    bm_serial_resource_table_request_t *resource_req_msg =
...
// TODO - This struct is defined in bcmp_messages.h, but that is not included in
// this repo yet. We should eventually include it, or have this struct moved
// into the common messages repo. Perhaps this can happen when we do this
// ticket: SC-195824
typedef struct {
...
bm_serial_send_resource_reply(uint64_t node_id,
  ...
  do {
    uint16_t message_len =
        sizeof(bm_serial_packet_t) + sizeof(bm_serial_resource_table_reply_t);
    ...
    bm_serial_packet_t *packet =
        _bm_serial_get_packet(BM_SERIAL_RESOURCE_REPLY, 0, message_len);

    if (!packet) {
      rval = BM_SERIAL_OUT_OF_MEMORY;
      break;
    }
    bm_serial_resource_table_reply_t *resource_req_msg =
...
bm_serial_error_e bm_serial_process_packet(bm_serial_packet_t *packet, size_t len) {
  ...
  do {
    ...
    case BM_SERIAL_CFG_CLEAR_REQ: {
      if (_callbacks.cfg_clear_request_fn) {
        BmConfigClearRequest *cfg_clear_req = (BmConfigClearRequest *)packet->payload;
        _callbacks.cfg_clear_request_fn(cfg_clear_req->header.target_node_id,
                                        cfg_clear_req->partition);
      }
      break;
    }
    case BM_SERIAL_CFG_CLEAR_RESP: {
      if (_callbacks.cfg_clear_response_fn) {
        BmConfigClearResponse *cfg_clear_resp = (BmConfigClearResponse *)packet->payload;
        _callbacks.cfg_clear_response_fn(cfg_clear_resp->header.source_node_id,
                                         cfg_clear_resp->partition, cfg_clear_resp->success);
      }
      break;
    }
    ...
    case BM_SERIAL_DEVICE_INFO_REQ: {
      if (_callbacks.bcmp_info_request_fn) {
        bm_serial_device_info_request_t *info_req =
            (bm_serial_device_info_request_t *)packet->payload;
    ...
    case BM_SERIAL_RESOURCE_REQ: {
      if (_callbacks.bcmp_resource_request_fn) {
        bm_serial_resource_table_request_t *resource_req =
            (bm_serial_resource_table_request_t *)packet->payload;
