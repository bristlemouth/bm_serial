...
typedef struct {
  ...
  bool (*tx_fn)(const uint8_t *buff, size_t len, bm_serial_message_t message);
  ...
  bool (*reboot_info_fn)(uint64_t node_id, uint32_t reboot_reason, uint32_t gitSHA,
  ...
  // Function called when there is a baud rate negotiation request
  bool (*baud_rate_negotiation_request_fn)(uint32_t baud);

  // Function called when there is a baud rate negotiation reply
  bool (*baud_rate_negotiation_reply_fn)(void);

  // Function called when an ack is received
  bool (*ack_fn)(void);
